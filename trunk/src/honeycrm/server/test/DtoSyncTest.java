...
public class DtoSyncTest extends TestCase {
	...
	/**
	 * Returns true if fields contains a field called fieldName TODO check field type as well. not
	 * just the name..
	 */
	...
	/**
	 * Determines whether the domain classes and the dtos are in sync, i.e. whether all DTO fields
	...
	public void testSync() {
		try {
			...
			Class[] domainClasses = ReflectionHelper.getClasses("honeycrm.server.domain");
			Class[] dtos = ReflectionHelper.getClasses("honeycrm.client.dto");

			boolean outOfSync = false;

			for (int i = 0; i < domainClasses.length; i++) {
				final Class domain = domainClasses[i];
				final String domainName = domain.getSimpleName();

				for (int k = 0; k < dtos.length; k++) {
					final Class dto = dtos[k];
					final String dtoName = dto.getSimpleName().substring(3); // throw away the first
																				// three characters.
																				// this should skip
																				// the "Dto" at the
																				// beginning
